...
> 路由是微前端中重要且复杂的模块。在微前端应用下，子应用既可作为独立应用运行在自己的路由系统下，又可作为子应用嵌入在主应用下加载，为了实现两种模式的无缝切换，Garfish 实现了一套路由机制保证用户在这两种模式中平滑过渡，你可以在 [Garfish 路由机制](/guide/concept/router) 中了解详细设计。
