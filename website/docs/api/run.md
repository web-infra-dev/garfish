...
### apps?
  ...
  ```
### beforeLoad?
  - Type: <span style={{color: '#ff5874', margin: '2px 6px'}}> async (appInfo: AppInfo) => false | undefined </span>
  ...
  ```
### afterLoad?
  - Type: <span style={{color: '#ff5874', margin: '2px 6px'}}> async (appInfo: AppInfo) => void </span>
    - 子应用加载完成后触发，此时子应用资源加载、处理完成，子应用沙箱运行时环境初始化完成；
    - 若加载过程中出现异常，将触发 [errorLoadApp](./run.md#errorLoadApp)；
    - 支持同步、异步调用；
  ...
  ```
### errorLoadApp?
  - Type: <span style={{color: '#ff5874', margin: '2px 6px'}}> (error: Error, appInfo: AppInfo) => void </span>
    - 子应用加载异常后触发
    - 可选。一旦设置该 hook，子应用加载错误不会throw 到文档流中，全局错误监听将无法捕获到；
    - 支持同步调用；
...
### beforeMount?
  - Type: <span style={{color: '#ff5874', margin: '2px 6px'}}> (error: Error, appInfo: AppInfo) => void </span>
    - 子应用渲染前触发；
    - 此时子应用资源准备完成，沙箱运行时环境初始化完成，准备开始渲染子应用 DOM 树；
...
### afterMount?
  - Type: <span style={{color: '#ff5874', margin: '2px 6px'}}> (error: Error, appInfo: AppInfo) => void </span>
    ...
    - 若挂载过程中出现异常，会触发 [errorMountApp](./run.md#errorMountApp)，同时会清除已创建的app 渲染容器 appContainer
...
### errorMountApp?
  ...
  ```
### beforeUnmount?
  - Type: <span style={{color: '#ff5874', margin: '2px 6px'}}> (appInfo: AppInfo) => void </span>
    - 子应用销毁前触发（当路由变化离开子应用的激活范围或主动触发销毁函数，将会触发应用的销毁）；
...
### errorUnmountApp?
  - Type: <span style={{color: '#ff5874', margin: '2px 6px'}}> (error: Error, appInfo: AppInfo)=> void </span>
    - 子应用销毁异常触发
    - 可选。一旦设置该 hook，子应用销毁错误不会向上throw 到文档流中，全局错误监听将无法捕获到；
    - 支持同步调用；
