...
import { getParameterByName, warn, __GARFISH_FLAG__, def } from '@garfish/utils';
...
try {
  const GarfishInstanceName = localStorage.getItem(DEBUG_GARFISH_TAG) || getParameterByName(DEBUG_GARFISH_TAG);
  if (GarfishInstanceName) {
    ...
    warn(`The current in the garfish debug mode, garfish instance name is "${uniqueName}"`);
  }
} catch(err) {
  warn(err);
}
