...
class HookTester {
	...
	async runForLoopAsync(result, method) {
...

		{
			...
			result[`${method}Mixed`] = await this.gainResult((cb) =>
				hook[method](cb)
			);
		}
	}
	...
	async runAsync(result, type) {
...

		{
			...
			result[`${type}SingleSync`] = await this.gainResult((cb) =>
				hook[type](cb)
			);
		}
...

		{
			...
			result[
				`${type}MultipleMixedError1WithArg`
			] = await this.gainResult((cb) => hook[type](42, cb));
		}

		{
			...
			result[
				`${type}MultipleMixedError2WithArg`
			] = await this.gainResult((cb) => hook[type](42, cb));
		}

		{
			...
			result[
				`${type}MultipleMixedError3WithArg`
			] = await this.gainResult((cb) => hook[type](42, cb));
		}
